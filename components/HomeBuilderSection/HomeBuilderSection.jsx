...
export default function ServiceShowcase() {
  ...
  return (
    <section className="relative w-full py-24 md:py-32 bg-[#F9F9F7] overflow-hidden">
      ...
      <div className="w-full px-6 lg:px-8 flex flex-col items-center text-center mb-20">
        ...
        <motion.h2 
          ...
          className="text-4xl md:text-5xl lg:text-7xl font-bold leading-[1.1] tracking-tight text-slate-900 max-w-5xl"
        >
      ...
      <motion.div 
        ...
        className="w-full px-4 md:px-12 lg:px-16 xl:px-24"
      >
...
function ServiceCard({ service }) {
  return (
    <motion.div
      ...
      className="group relative w-full md:w-[calc(50%-1.5rem)] lg:w-[calc(33.333%-2rem)] h-[400px] md:h-[500px] bg-white rounded-[2rem] overflow-hidden shadow-sm hover:shadow-2xl hover:shadow-emerald-900/20 hover:-translate-y-2 transition-all duration-500 ease-out hover:bg-emerald-600"
    >
      <div className="absolute top-0 left-0 right-0 p-8 z-20 flex justify-between items-start">
        <h3 className="text-2xl md:text-4xl font-bold text-slate-900 group-hover:text-white transition-colors duration-300 max-w-[80%] leading-tight">
          {service.title}
        </h3>
        
        <div className="w-10 h-10 rounded-full bg-[#F8F7F4] group-hover:bg-white/20 flex items-center justify-center transition-all duration-300 group-hover:rotate-90 shrink-0">
          <X className="w-5 h-5 text-slate-400 group-hover:text-white" strokeWidth={2.5} />
        </div>
      </div>

      <div className="absolute bottom-0 left-0 right-0 h-[60%] w-full overflow-hidden">
